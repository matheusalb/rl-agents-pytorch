...
if __name__ == "__main__":
    ...
    checkpoint_atks = []
    for p in ls_atk:
        checkpoint_atks.append(torch.load(args.path_atk +'/'+ p))

    checkpoint_gks = []
    for p in ls_gk:
        checkpoint_gks.append(torch.load(args.path_gk +'/'+ p))
    ...
    try:
        while n_grads < hp.TOTAL_GRAD_STEPS:
            ...
            A_next_v = tgt_pi(S_next_v_eval)  # Get an Bootstrap Action for S_next
            Q_next_v = tgt_Q(S_next_v_eval, A_next_v)  # Bootstrap Q_next
            Q_next_v[dones == 1.] = 0.0  # No bootstrap if transition is terminal
            # Calculate a reference Q value using the bootstrap Q
            Q_ref_v_eval = r_v + Q_next_v * (hp.GAMMA**hp.REWARD_STEPS)
            Q_loss_v = F.mse_loss(Q_v, Q_ref_v_eval.detach())
            Q_loss_v.backward()
            Q_opt.step()
            metrics["train/loss_Q_eval"] = Q_loss_v.cpu().detach().numpy()

            # train actor - Maximize Q value received over every S
            pi_opt.zero_grad()
            A_cur_v_eval = pi(S_v)
            pi_loss_v = -Q(S_v, A_cur_v_eval)
            pi_loss_v = pi_loss_v.mean()
            if n_grads > 50000:
                pi_loss_v.backward()
                pi_opt.step()
            metrics["train/loss_pi_eval"] = pi_loss_v.cpu().detach().numpy()
