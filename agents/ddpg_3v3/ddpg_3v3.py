...
def data_func(
    ...
    with torch.no_grad():
        while not finish_event_m.is_set():
            ...
            pi_enemy_gk = pi_gks[np.random.randint(0, len(pi_gks))] if n_enemy_gk == 1 else None
            ids_pi_enemy_atks = np.random.choice(len(pi_atks), size=n_enemy_atks)
            pi_enemy = [None]*3
            ...
            # select ally team
            n_allies_atk = np.random.randint(0, n_enemy_atks)
            ids_pi_ally_atk = np.random.choice(len(pi_atks), size=n_allies_atk)
            pi_allies = [None]*2
